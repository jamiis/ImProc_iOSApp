...
@interface OFVideoViewController () <UIGestureRecognizerDelegate>
@end

@interface OFVideoViewController (InternalMethods)
...
@interface OFVideoViewController (OFVideoCaptureManagerDelegate) <OFVideoCaptureManagerDelegate>
@end
...
- (void)loadView
{
    NSLog(@"loadView in OFVideoViewController");
    
    [super loadView];
    
    // allocate buttons
    cameraToggleButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    cameraToggleButton.frame = CGRectMake(0.0, 50.0f, 30.0f, 30.0f);

    recordButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    recordButton.frame = CGRectMake(0.0, 80.0f, 30.0f, 30.0f);
    
    stillButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    stillButton.frame = CGRectMake(0.0, 110.0f, 30.0f, 30.0f);
...
- (void)viewDidLoad
{
    NSLog(@"Toggle camera button title");
...
//    [[self stillButton] setTitle:NSLocalizedString(@"Photo", @"Capture still image button title")];
	...
	if ([self captureManager] == nil) {
		OFVideoCaptureManager *manager = [[OFVideoCaptureManager alloc] init];
		[self setCaptureManager:manager];
		[manager release];
		
		[[self captureManager] setDelegate:self];
        
        NSLog(@"videoPreviewView description:: %@",videoPreviewView.description);
...
@implementation OFVideoViewController (InternalMethods)
...
@implementation OFVideoViewController (OFVideoCaptureManagerDelegate)
